...
enum {
  ...
  CHECKERS_PUZZLE_MAX_SELF_PLAY_PLIES = 1000,
...
static gboolean checkers_puzzle_try_forced_mistake_candidates(const Game *before_mistake_game,
                                                              ...
                                                              CheckersPuzzleEmitContext *emit_context);
...
static gboolean checkers_puzzle_copy_sgf_node_properties(const SgfNode *source, SgfNode *dest) {
  g_return_val_if_fail(source != NULL, FALSE);
  g_return_val_if_fail(dest != NULL, FALSE);

  g_autoptr(GPtrArray) idents = sgf_node_copy_property_idents(source);
  g_return_val_if_fail(idents != NULL, FALSE);

  for (guint i = 0; i < idents->len; ++i) {
    const char *ident = g_ptr_array_index(idents, i);
    const GPtrArray *values = sgf_node_get_property_values(source, ident);
    ...
    for (guint j = 0; j < values->len; ++j) {
      ...
      g_return_val_if_fail(value != NULL, FALSE);
      if (!sgf_node_add_property(dest, ident, value)) {
        return FALSE;
      }
    }
  }
...
static SgfTree *checkers_puzzle_build_forced_source_tree(const GGameCreatePuzzlesMoveContext *move_context,
                                                         CheckersColor color,
                                                         const CheckersMove *move) {
  g_return_val_if_fail(move_context != NULL, NULL);
  g_return_val_if_fail(move_context->source_tree != NULL, NULL);
  g_return_val_if_fail(move_context->main_line != NULL, NULL);
  g_return_val_if_fail(move_context->node_index < move_context->main_line->len, NULL);
  ...
  const SgfNode *source_root = sgf_tree_get_root(move_context->source_tree);
  if (source_root == NULL || !checkers_puzzle_copy_sgf_node_properties(source_root, root)) {
    return NULL;
  }

  for (guint i = 1; i < move_context->node_index; ++i) {
    const SgfNode *source_node = g_ptr_array_index(move_context->main_line, i);
    SgfNode *dest_node = (SgfNode *)sgf_tree_append_node(tree);
    g_return_val_if_fail(source_node != NULL, NULL);
    g_return_val_if_fail(dest_node != NULL, NULL);
    if (!checkers_puzzle_copy_sgf_node_properties(source_node, dest_node)) {
      return NULL;
    }
  }

  SgfNode *mistake_node = (SgfNode *)sgf_tree_append_node(tree);
  g_return_val_if_fail(mistake_node != NULL, NULL);
  g_autoptr(GError) move_error = NULL;
  if (!sgf_move_props_set_move(mistake_node, checkers_puzzle_sgf_color(color), move, &move_error)) {
    g_debug("Failed to set forced mistake source move: %s",
...
static gboolean checkers_puzzle_emit_validated_candidate(const GameState *start_state,
  ...
  g_autofree char *puzzle_path = checkers_puzzle_build_indexed_path(emit_context->output_dir, "puzzle", index);
  ...
  if (emit_context->save_games && source_tree != NULL) {
    game_path = checkers_puzzle_build_indexed_path(emit_context->output_dir, "game", index);
    g_return_val_if_fail(game_path != NULL, FALSE);
    g_autoptr(GError) save_error = NULL;
    if (!ggame_create_puzzles_runner_save_source_game(game_path, source_tree, &save_error)) {
      g_debug("Failed to save source game file %s: %s",
              game_path,
...
static gboolean checkers_puzzle_try_forced_mistake_candidates(const Game *before_mistake_game,
  ...
  CheckersScoredMoveList analysis = {0};
  if (!checkers_puzzle_analyze_moves_with_shared_tt(before_mistake_game,
                                                    emit_context->depth,
                                                    emit_context->tt,
                                                    &analysis)) {
    return FALSE;
  ...
  for (guint i = 0; i < candidates->len && *emit_context->out_emitted < emit_context->max_total_emitted; ++i) {
    ...
    g_autoptr(SgfTree) candidate_source_tree =
        checkers_puzzle_build_forced_source_tree(move_context,
                                                 before_mistake_game->state.turn,
                                                 &candidate->move);
    if (candidate_source_tree == NULL) {
      return FALSE;
    }
...
static gboolean checkers_puzzle_consider_runner_move(const GGameCreatePuzzlesMoveContext *move_context,
                                                     gpointer user_data,
                                                     gboolean *out_stop,
                                                     GError ** /*error*/) {
  g_return_val_if_fail(move_context != NULL, FALSE);
  ...
  CheckersPuzzleEmitContext *emit_context = user_data;
  if (*emit_context->out_emitted >= emit_context->max_total_emitted) {
    *out_stop = TRUE;
    return TRUE;
  }

  const Game *before = move_context->position_before;
  const Game *after = move_context->position_after;
  const CheckersMove *played = move_context->played_move;
  checkers_puzzle_run_stats.moves_analyzed++;

  if (emit_context->try_forced_mistakes) {
    if (!checkers_puzzle_try_forced_mistake_candidates(before, played, move_context, emit_context)) {
      return FALSE;
    }
    if (*emit_context->out_emitted >= emit_context->max_total_emitted) {
      *out_stop = TRUE;
  ...
  gboolean is_mistake = checkers_puzzle_position_follows_a_serious_mistake(before,
                                                                           ...
                                                                           &mistake_delta);
  if (is_mistake) {
    (void)checkers_puzzle_emit_candidate_if_valid(after,
                                                  emit_context,
                                                  mistake_delta,
                                                  move_context->source_tree,
                                                  emit_context->out_emitted);
  } else {
...
static gboolean checkers_puzzle_emit_from_tree(SgfTree *tree,
  ...
  GGameCreatePuzzlesRunnerConfig runner_config = {
    .backend = checkers_puzzle_backend(),
    .variant = variant,
    .self_play_depth = CHECKERS_PUZZLE_SELF_PLAY_DEPTH,
    .max_self_play_plies = CHECKERS_PUZZLE_MAX_SELF_PLAY_PLIES,
  };
  CheckersPuzzleEmitContext emit_context = {
    .depth = best_move_depth,
    .tt = tt,
    .try_forced_mistakes = try_forced_mistakes,
  ...
  if (!ggame_create_puzzles_runner_analyze_tree(&runner_config,
                                                tree,
                                                checkers_puzzle_consider_runner_move,
                                                &emit_context,
                                                &runner_error)) {
    g_debug("Checkers source game analysis failed: %s",
...
int checkers_create_puzzles_main(int argc, char **argv, guint default_depth) {
  ...
  const GameBackendVariant *variant = checkers_puzzle_ruleset_variant(options.ruleset);
  if (variant == NULL) {
    g_printerr("Missing ruleset variant\n");
    return 1;
  }

  if (arg_type == CHECKERS_PUZZLE_ARG_FILE) {
    checkers_puzzle_run_stats.games_processed = 1;
    g_autoptr(SgfTree) source_tree = NULL;
    g_autoptr(GError) load_error = NULL;
    if (!sgf_io_load_file(options.arg, &source_tree, &load_error)) {
      g_printerr("Failed to load source game from file %s: %s\n",
                 options.arg,
    ...
    guint emitted = 0;
    if (!checkers_puzzle_emit_from_tree(source_tree,
                                        variant,
                                        options.depth,
                                        analysis_tt,
  ...
  while (generated < wanted && attempts < max_attempts) {
    GGameCreatePuzzlesRunnerConfig runner_config = {
      .backend = checkers_puzzle_backend(),
      .variant = variant,
      .self_play_depth = CHECKERS_PUZZLE_SELF_PLAY_DEPTH,
    ...
    if (source_tree == NULL) {
      g_printerr("Self-play generation failed: %s\n",
                 runner_error != NULL ? runner_error->message : "unknown error");
      return 1;
    }
    checkers_puzzle_run_stats.games_processed++;
  ...
  checkers_puzzle_print_final_report();
  if (generated < wanted) {
    g_printerr("Only generated %u of %u requested checkers puzzles after %u source games\n",
               generated,
               wanted,
               attempts);
    return 1;
