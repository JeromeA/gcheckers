...
static gboolean ggame_app_paths_is_directory(const char *path) {
...
char *ggame_app_paths_find_data_subdir(const char *env_name, const char *subdir_name) {
  ...
  if (ggame_app_paths_is_directory(user_dir)) {
  ...
  if (system_dirs != NULL) {
    for (guint i = 0; system_dirs[i] != NULL; i++) {
      ...
      if (ggame_app_paths_is_directory(system_dir)) {
  ...
  if (ggame_app_paths_is_directory(local_dir)) {
