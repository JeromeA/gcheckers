...
static void ghomeworlds_on_activate(GtkApplication *app, gpointer /*user_data*/) {
  ...
  GtkWidget *body = gtk_label_new("Milestone 1 wires build-time GAME=homeworlds selection and a stub backend.\n"
...
int main(int argc, char **argv) {
  GtkApplication *app = gtk_application_new("io.github.jeromea.ghomeworlds", G_APPLICATION_DEFAULT_FLAGS);
  int status = 0;

  g_return_val_if_fail(GTK_IS_APPLICATION(app), 1);

  g_signal_connect(app, "activate", G_CALLBACK(ghomeworlds_on_activate), NULL);
