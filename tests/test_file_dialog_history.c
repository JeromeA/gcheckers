...
static const char *gcheckers_file_dialog_history_key = "sgf-last-folder";
...
static void test_file_dialog_history_round_trip(void) {
  ...
  g_autoptr(GFile) initial_folder =
      gcheckers_file_dialog_history_get_initial_folder(settings, gcheckers_file_dialog_history_key);
  g_assert_null(initial_folder);

  g_autoptr(GFile) file = g_file_new_for_path("/tmp/gcheckers-puzzles/example.sgf");
  g_assert_true(gcheckers_file_dialog_history_remember_parent(settings, gcheckers_file_dialog_history_key, file));
