...
static void test_sgf_view_connectors(void) {
  ...
  GtkWidget *tree_box = gtk_scrolled_window_get_child(GTK_SCROLLED_WINDOW(root));
  g_assert_nonnull(tree_box);

  GtkWidget *row = gtk_widget_get_first_child(tree_box);
  g_assert_nonnull(row);

  g_assert_cmpint(sgf_view_count_children(row), ==, 5);

  GtkWidget *child = gtk_widget_get_first_child(row);
  g_assert_true(GTK_IS_BUTTON(child));
  child = gtk_widget_get_next_sibling(child);
  g_assert_true(GTK_IS_DRAWING_AREA(child));
  child = gtk_widget_get_next_sibling(child);
  g_assert_true(GTK_IS_BUTTON(child));
  child = gtk_widget_get_next_sibling(child);
  g_assert_true(GTK_IS_DRAWING_AREA(child));
